...
struct PostProcessRecord {
    ...
    blend_weight: f32,
    // Exported so the authored method is visible in the manifest; Bevy has one
    // parametric bloom, so only the intensity is read.
    #[serde(default, rename = "bloom_method")]
    _bloom_method: Option<String>,
    #[serde(default)]
...
fn legacy_zorah_post_process(level: &str) -> Option<PostProcessRecord> {
    ...
    Some(PostProcessRecord {
        ...
        priority: 0.0,
        blend_weight: 1.0,
        _bloom_method: bloom_method.map(String::from),
        bloom_intensity,
        _film_slope: None,
        _film_toe: None,
        _film_shoulder: None,
...
/// histogram, 3 EV/s brightening, 1 EV/s darkening. UE's bias becomes a flat
/// exposure compensation: positive brightens on both sides. The calibration
...
/// at `0.18 * 2^bias`, while Bevy's histogram lands it at `2^bias`. Restir's
...
fn resolved_bloom(post_process: Option<&PostProcessRecord>) -> Bloom {
    ...
    // UE's BM_FFT (aperture convolution) and BM_SOG (sum of Gaussians) both
    // land on Bevy's one parametric mip-chain bloom; the method is not
    // representable, only the intensity carries over.
    bloom
}
...
fn setup(
    ...
    // `Linear` rather than `None`: the albedo capture still wants the exposure
    ...
    let mut camera = commands.spawn((
        ...
        // `Tonemapping::AcesFitted` is the closest operator by curve shape; the
        // authored Film* parameters ride along in the manifest so the choice
        // can be revisited if Bevy grows a parameterizable ACES curve.
        tonemapping,
        bloom,
        ZorahCamera,
        FreeCamera {
...
mod tests {
    ...
    fn ue_post_process_bloom_drives_bevy_bloom() {
        ...
        assert_eq!(green_house.intensity, Bloom::NATURAL.intensity);
        assert_eq!(resolved_bloom(None).intensity, Bloom::NATURAL.intensity);
    }

    #[test]
    fn ue_sum_of_gaussians_bloom_keeps_its_intensity() {
        let post_process = PostProcessRecord {
            _bloom_method: Some("BM_SOG".into()),
            bloom_intensity: Some(0.675),
            ..legacy_zorah_post_process("Restir_Level").unwrap()
        };

        assert_eq!(resolved_bloom(Some(&post_process)).intensity, 0.675);
    }
