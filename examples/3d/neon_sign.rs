...
//!   reported from `WindowSurfaceTransfers::resolved`.
//!
//! ## Controls
//!
//! | Key             | Action                                                          |
...
use bevy::{
    ...
    window::{DisplayTarget, DisplayTransfer, PrimaryWindow, WindowSurfaceTransfers},
...
struct NeonScene {
    ...
    /// backend actually negotiated) comes from `WindowSurfaceTransfers::resolved`.
    output: OutputMode,
    headlights_on: bool,
...
/// it reads `WindowSurfaceTransfers` and requests the best the surface advertises
...
fn update_hud(
    ...
    surface: Single<Option<&WindowSurfaceTransfers>, With<PrimaryWindow>>,
    mut hud: Single<&mut Text, With<HudText>>,
) {
    // `WindowSurfaceTransfers` is absent until the first surface configuration
    // and its `resolved` lags requests by one frame; treat absence as pending
    // rather than lying about the output mode.
