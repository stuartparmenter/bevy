//! Importance sampling for the environment map light ([`ExtractedEnvironmentMapLight`]).
//!
//! `sample_environment_map_light` gives the radiance: the cubemap's mip 0 with `intensity` and
//! `rotation` applied as in `bevy_pbr`'s raster path and the skybox. Sampling it uses an
//! importance pyramid: the six cube faces on a
...
//! `GeneratedEnvironmentMapLight` source, which may be rewritten on the GPU each frame (the
//! atmosphere renders into one). A map modified while unbound is rebuilt on its next binding; one modified while
...
//!
//! Without an environment map light there is no environment light source at all: no entry in the
//! light list, so its selection probability is 0, and a zero intensity, so a missed ray collects 0.
//!
//! [`ExtractedEnvironmentMapLight`]: super::extract::ExtractedEnvironmentMapLight
...
use bevy_render::{
    ...
    render_resource::{
        ...
        TextureFormat, TextureId, TextureSampleType, TextureUsages, TextureView,
...
/// The importance pyramid Solari samples the environment through.
#[derive(Resource)]
pub struct EnvironmentImportanceMaps {
    /// The pyramid for the environment cubemap, re-targeted per asset and reallocated when the
    /// source's face size asks for a different [`importance_face_size`]; `source` is `None` while
    /// unused.
...
impl EnvironmentImportanceMaps {
    /// Targets the pyramid at the `cubemap` asset, scheduling a rebuild when the asset or its GPU
    /// texture differs from what the pyramid was last built from, or when `contents_changed` says
    /// the texels may differ in place (the asset was re-prepared this frame, or the map is
    /// regenerated every frame). A retarget reallocates the pyramid first when the cubemap's face
    /// size asks for a different pyramid size.
    pub(crate) fn request(
        &mut self,
        cubemap: AssetId<Image>,
        gpu: &GpuImage,
        contents_changed: bool,
        device: &RenderDevice,
        queue: &RenderQueue,
    ) {
        let source = (cubemap, gpu.texture.id());
        ...
        self.needs_build |= retargeted || contents_changed;
    }
...
pub fn build_environment_importance_maps(
    ...
    let build_bind_group = render_device.create_bind_group(
        ...
        &BindGroupEntries::sequential((
            &source.texture_view,
            scene_bindings.environment_map_light_sampler(),
            &pyramid.mip0_view,
            build_constants,
        )),
    );
...
mod tests {
    ...
    fn shader_constants_match_rust() {
        ...
        assert!(bindings.contains("@group(0) @binding(16) var environment_map: texture_cube<f32>;"));
        assert!(bindings.contains("@group(0) @binding(17) var environment_map_sampler: sampler;"));
        assert!(bindings
            .contains("@group(0) @binding(20) var environment_importance_map: texture_2d<f32>;"));
