...
use super::{
    ...
    RaytracingGeometryTopologyGeneration, RaytracingInstanceTag, RaytracingMesh3d,
};
use bevy_ecs::{
    entity::Entity,
...
impl RaytracingSceneBindings {
    /// Marks both translation tables as consumed so the next frame can advance
    /// the light and instance snapshots.
...
pub struct InstanceChanges<'w, 's> {
    ...
    removed_instance_tags: RemovedComponents<'w, 's, RaytracingInstanceTag>,
}
...
pub fn prepare_raytracing_scene_resources(
    ...
    // Advance snapshots before updating the scene if lighting consumed the tables.
    // Also advance when there is no consumer, to avoid accumulating changes indefinitely.
    ...
    bindings.instances.pending_refresh.extend(
        instance_changes
            ...
            .chain(instance_changes.removed_instance_tags.read()),
    );
    ...
    // Update translations for removed or reassigned instance slots
