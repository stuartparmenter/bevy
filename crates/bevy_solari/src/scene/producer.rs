...
/// per-producer params uploads stay ordered before their dispatches. This
/// replaces one `queue.submit` per producer per frame (each with its own
/// pending-writes flush) with one total.
///
...
/// Submits the shared producer encoder once anything has been recorded into
/// it: the producers' fill passes and this frame's geometry BLAS builds.
