...
impl RenderResources {
    ...
    pub(crate) fn unpack_into(
        ...
        // Read out of the render world (inserted during `RenderPlugin::build`)
        // rather than taken as a parameter, so the device-loss reinit path
        // rebuilds the cache with the same working color space.
