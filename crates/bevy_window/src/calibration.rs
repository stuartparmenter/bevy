...
//! renderer encodes for. These types carry the other three provenances: what
//! the display can do ([`MonitorDisplayCapability`]), what it is doing right
//! now ([`WindowDisplayState`]), and which fields the engine may fill in
//! ([`DisplayCalibrationPolicy`]). The merged result is
...
///
/// Absence means "can't tell", never "SDR". Every field is `Option` for the
/// same reason: a platform reports whatever subset it can. Luminance is in
...
pub struct MonitorDisplayCapability {
    ...
    /// [`max_nits`] when content fills the frame with highlights, so tone
    /// mapping targets a level the panel can hold. Only DXGI reports it, so it
    /// is `None` under Metal and Vulkan even on a panel that has a full-frame
    /// ceiling; fall back to [`max_nits`] there.
    ///
...
/// a fresh reading only once it moves past a relative epsilon, so
...
/// It is absent until the window's first successful read, and never appears on
/// a platform that reports nothing (Vulkan outside Win32, GLES/WebGL, Android).
#[derive(Component, Debug, Clone, Copy, Default, PartialEq)]
#[cfg_attr(
    feature = "bevy_reflect",
    derive(Reflect),
...
pub struct WindowDisplayState {
    ...
    /// tell here", never "SDR". Peak-aware tone mapping targets it: GT7's HDR
    /// ceiling is `peak / paper_white`, which auto-resolves to this multiplier.
    ...
    /// `DISPLAYCONFIG_SDR_WHITE_LEVEL`; it moves with the SDR-content
    /// brightness slider). `None` on the Apple EDR and web paths. Feeds the
...
/// [`EffectiveDisplayTarget`] may diverge from the authored target. A `true`
/// field takes the sensed value when one is available and the authored value
/// when nothing is sensed; a `false` field keeps the authored value verbatim.
/// The default is all-`false`, so the effective target equals the authored
...
impl DisplayCalibrationPolicy {
    ...
    ///
    /// When `false` the resolver is an identity pass, so the render-side poll
    /// skips display sensing for the window.
...
///
/// Calibration UIs read this to show why a value is what it is.
...
///
/// [`DisplayTarget::transfer`] has no entry: it is never auto-resolved.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Hash)]
#[cfg_attr(
    feature = "bevy_reflect",
    derive(Reflect),
...
/// when the resolved value changes. Resolution runs in the main world, before
/// extraction, so the identity case (an all-manual policy) has zero frame lag
/// and a user-set-HDR project shows HDR on its first frame with no SDR pop.
/// The render pipeline reads [`target`](Self::target) in place of the raw
/// [`DisplayTarget`]; if the component is removed it falls back to
/// [`DisplayTarget::SDR_SRGB`], as it does for a missing [`DisplayTarget`].
